...
if(isset($_POST['reset'])){
    ...
    if($pass1 == $pass2){
        $hashed_password = password_hash($pass1, PASSWORD_ARGON2ID);
    ...
    // Update password
    if(empty($error)){
        $query = $pdo->prepare("UPDATE utilisateurs SET password = ? WHERE email = ?");
        $query ->execute([$hashed_password, $email]);

        $msg = 'Successfully updated your password! <a class="btn btn-danger" href="login.php">Log In</a>';
...
<div class="container mt-5 mb-5" style="margin: 0 auto; width: 60%;">
    ...
    <?php if(isset($msg)){echo '<div class="logocheck" style="text-align: center; padding-top: 10%">
    ...
    if(!isset($expired) && isset($_GET['code'])){
        ...
        <form action="reset_password.php" method="post">
            ...
            <input type="password" name="pass2" class="form-control mt-2" placeholder="Repeat new password..." required>
            <input type="submit" name="reset" value="Reset Password" class="form-control btn btn-danger mt-2">
        </form>
        ';
    }
